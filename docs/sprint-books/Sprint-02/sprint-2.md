...
> **Status**: ✅ COMPLETE (post-sprint addition: SEO validation pipeline added Jul 29)
...
├── .github/workflows/seo-check.yml    # Lighthouse + SEO validation on PR
├── .lighthouserc.js                   # Lighthouse CI config (SEO >= 90, a11y >= 85, perf >= 70)
└── scripts/seo-check.mjs              # Static SEO checks (title, meta, H1, alt, robots, links)
```
...
| SEO | Lighthouse CI + basic SEO checks (title, meta, H1, alt, robots.txt, broken links) | 3 min | Block |
...
A dedicated `seo-check.yml` workflow runs on every PR to `main` that touches Angular files:

1. **Lighthouse CI** — Audits the built Angular app for SEO (>= 90), Accessibility (>= 85), Performance (>= 70)
2. **Basic SEO Checks** — Validates title, meta description, viewport, H1, image alt attributes, robots.txt, sitemap.xml (optional), and internal broken links
3. **Fail rules** — CI blocks only on: missing `<title>`, missing `<meta name="description">`, Lighthouse SEO score < 90, broken internal links (404)
4. **Report** — Lighthouse results and HTML report saved as CI artifacts

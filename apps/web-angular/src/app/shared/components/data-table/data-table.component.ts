...
export class DataTableComponent<T = any> implements OnInit, OnDestroy {
  ...
  formatCellValue(row: T, col: TableColumn): string {
    ...
    switch (col.pipe) {
      case 'currency': return '₹' + Number(val).toLocaleString('en-IN');
      case 'percent': return (Number(val) > 0 ? '+' : '') + Number(val).toFixed(2) + '%';
  ...
  trackByColKey(_index: number, col: TableColumn): string { return col.key; }
